...
public class MetadataTests {
    ...
    public void Should_Succeed_When_SerializingTokenMetadata() throws IOException {
        ...
        String jsonStrRepr = objectMapper.writeValueAsString(metadata);
        log.info(jsonStrRepr);

        MetadataCreator.signMetadata(metadata, skey, "contact");
        jsonStrRepr = objectMapper.writeValueAsString(metadata);
        log.info(jsonStrRepr);

        MetadataCreator.validateMetadata(metadata, vkey, false);
    }

    @Test
    public void Should_Succeed_When_SigningSingleWitnessReferenceData() throws IOException {
    ...
    @Test
    public void Should_Succeed_When_SigningMultiWitnessReferenceData() throws IOException {
        final String assetName = "myassetname";
        final ObjectMapper jsonMapper = new ObjectMapper();
        final KeyTextEnvelope signingEnvelope = jsonMapper.readValue(RESOURCE_DIRECTORY.resolve("policy.skey").toFile().getAbsoluteFile(), KeyTextEnvelope.class);
        final Key signingKey = Key.fromTextEnvelope(signingEnvelope, KeyType.POLICY_SIGNING_KEY_ED25519);
